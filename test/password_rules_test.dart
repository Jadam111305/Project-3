...
String winningPassword(PasswordGame game) {
  final base =
      'David! Washington ${game.verse.language} ${game.math.answer} helium 😀 ${game.today} 72 .pdf .png .txt ';
...
void main() {
  test('temperature matches the signed whole number, not a substring', () {
    ...
    expect(game.passes(16, '72', false), isFalse);
    game.temperatureF = -5;
    expect(game.passes(16, 'temp-5F', false), isTrue);
    for (final password in ['5', '15', '-50', '-5.5', '1.5', '35']) {
      expect(game.passes(16, password, false), isFalse);
    }
    game.temperatureF = 72;
    expect(game.passes(16, 'temp72F', false), isTrue);
    expect(game.passes(16, '-72', false), isFalse);
    expect(game.passes(16, '172', false), isFalse);
  });
  ...
  test('file extensions require three distinct known extensions with dots', () {
    ...
    final game = PasswordGame();
    expect(game.passes(18, '.PDF.png.txt', false), isTrue);
    expect(game.passes(18, '.pdf.PDF.txt', false), isFalse);
  });
  test('all 100 verse and math combinations can satisfy every rule', () {
  ...
  test(
    ...
    () {
      final game = PasswordGame();
      expect(game.passes(0, '123456😀', false), isFalse);
      expect(game.passes(0, '1234567👨‍👩‍👧‍👦', false), isTrue);
  ...
  test('math and self-length require complete digit groups', () {
    ...
    final game = PasswordGame();
    expect(game.passes(12, 'abcde7!', false), isTrue);
    expect(game.passes(12, 'abcde7!!', false), isFalse);
  });
  ...
  test('names accept full spellings without case sensitivity', () {
    ...
    for (final element in elementNames) {
      expect(game.passes(8, element.toUpperCase(), false), isTrue);
    }
    expect(game.passes(8, 'He', false), isFalse);
    for (final president in presidentSurnames) {
      expect(game.passes(5, president.toUpperCase(), false), isTrue);
    }
    expect(game.passes(5, 'notapresident', false), isFalse);
  });

  test('date accepts leading zeros and updates across midnight', () {
    var now = testDate;
    final game = PasswordGame(now: () => now);
    expect(game.passes(13, '09/17/2026', false), isTrue);
    expect(game.passes(13, '9/17/2026', false), isTrue);
    for (final invalid in [
      ...
      expect(game.passes(13, invalid, false), isFalse);
    }
    now = now.add(const Duration(days: 1));
    expect(game.passes(13, '9/17/2026', false), isFalse);
    expect(game.passes(13, '9/18/2026', false), isTrue);
  });
}
